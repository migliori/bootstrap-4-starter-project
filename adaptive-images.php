...
$document_root  = rtrim($_SERVER['DOCUMENT_ROOT'], DIRECTORY_SEPARATOR);
...
function generateImage($source_file, $cache_file, $resolution)
{
  ...
  // Check the image dimensions
    ...
    $width        = $dimensions[0];
    $height       = $dimensions[1];
    $img_screen_ratio = 1; // default image width is 100% of $resolution

    // check if filename matches -[xs|s|md|l]-x, where x is a number percent of $resolution. ie: image-md-50.jpg
    if (preg_match('`-(xs|sm|md|lg)-([0-9]{2})\.`', $source_file, $out)) {
        $breakpoint = $breakpoints[$out[1]];
  ...
  // Do we need to downscale the image?
    if ($width <= $resolution * $img_screen_ratio) { // no, because the width of the source image is already less than the client width
